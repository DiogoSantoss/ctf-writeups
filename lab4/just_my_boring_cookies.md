...
This challenges consists in grabbing the user' own cookie.
By searching for `<script>alert(1)</script>` in the search bar, the browser will display an alert, which
reveals that injecting JavaScript code is possible.
...
Alternatively, type the link directly in the browser's address bar `http://mustard.stt.rnl.tecnico.ulisboa.pt:22251/?search=<script>alert(document.cookie)</script>`.
